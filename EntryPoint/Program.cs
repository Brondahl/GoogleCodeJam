...
//      {
//        var nextSemiPrime = input.Numbers[locatedIndex + 1];
//        var identifiedPrimeFactor = underlyingPrimes[locatedIndex + 1];
//        underlyingPrimes[locatedIndex + 2] = nextSemiPrime / identifiedPrimeFactor;
//      }

//      for (int index = locatedIndex; index > 1; index--)
//      {
//        var prevSemiPrime = input.Numbers[locatedIndex - 1];
//        var identifiedPrimeFactor = underlyingPrimes[locatedIndex];
//        underlyingPrimes[locatedIndex - 1] = prevSemiPrime / identifiedPrimeFactor;
//      }

//      var translationLookup = underlyingPrimes.OrderBy(x => x).Select((x, index) => new { x, index }).ToDictionary(
//        xWithIndex => xWithIndex.x,
//        xWithIndex => "ABCDEFGHIJKLMNOOPQRSTUVWXYZ"[xWithIndex.index]
//      );
